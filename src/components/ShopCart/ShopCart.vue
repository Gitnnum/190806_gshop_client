  ...
  <div>
    <div class="shopcart">
        ...
        <div class="content-left" @click="toggleShow">
            ...
            <div class="num" v-show="cartFoods.length>0">{{totalCount}}</div>
            </div>
            <div class="price" :class="{highlight:cartFoods.length>0}">￥{{totalMoney}}</div>
...
<script type="text/ecmascript-6">
  ...
  export default {
		...
		computed: {
			...
			payClass () {
				const {totalMoney} = this
				const {minPrice} = this.info
				return totalMoney > minPrice ? 'enough' : 'not-enough'
			},
			payText () {
				const {totalMoney} = this
				const {minPrice} = this.info
				if(totalMoney === 0){
					...
					return `还差${minPrice-totalMoney}元起送`
				}else{
					return '去结算'
				}
			},
			// 列表是否显示  两个条件，isShow  与   totalCount  
			listShow () {
